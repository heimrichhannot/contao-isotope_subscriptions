...
$GLOBALS['TL_DCA']['tl_iso_subscription_archive'] = array
(
	...
	(
		'dataContainer'    => 'Table',
		'ctable'           => array('tl_iso_subscription'),
		'switchToEdit'     => true,
		'enableVersioning' => false,
		'onload_callback' => array
		(
			array('tl_iso_subscription_archive', 'checkPermission'),
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		(
			...
			(
				...
				'href'            => 'act=edit',
				'icon'            => 'header.gif',
				'button_callback' => array('tl_iso_subscription_archive', 'editHeader'),
			...
			case 'edit':
				...
				if (!in_array(Input::get('id'), $root))
				{
					...
					if (is_array($arrNew['tl_iso_subscription_archive']) && in_array(Input::get('id'), $arrNew['tl_iso_subscription_archive']))
					{
						...
						{
							$objUser = $objDatabase->prepare("SELECT subscriptions, subscriptionp FROM tl_user WHERE id=?")
								->limit(1)
								->execute($objUser->id);
							...
							if (is_array($arrsubscriptionp) && in_array('create', $arrsubscriptionp))
							{
								...
								$objDatabase->prepare("UPDATE tl_user SET subscriptions=? WHERE id=?")
									->execute(serialize($arrsubscriptions), $objUser->id);
							}
						}
						...
						{
							$objGroup = $objDatabase->prepare("SELECT subscriptions, subscriptionp FROM tl_user_group WHERE id=?")
								->limit(1)
							...
							if (is_array($arrsubscriptionp) && in_array('create', $arrsubscriptionp))
							{
								...
								$objDatabase->prepare("UPDATE tl_user_group SET subscriptions=? WHERE id=?")
									->execute(serialize($arrsubscriptions), $objUser->groups[0]);
							}
						}
