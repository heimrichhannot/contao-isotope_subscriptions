...
$GLOBALS['TL_DCA']['tl_iso_subscription'] = array
(
	'config'   => array
	(
		'dataContainer'     => 'Table',
		...
		(
			array('tl_iso_subscription', 'setDateAdded')
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		(
			...
			'child_record_callback' => array('tl_iso_subscription', 'listSubscriptions')
		),
		'global_operations' => array
		(
			'export' => array
			(
				'label'      => &$GLOBALS['TL_LANG']['tl_iso_subscription']['export'],
				'href'       => 'key=export',
				'class'      => 'header_export_entities',
		...
		(
			...
			(
				...
				'button_callback' => array('tl_iso_subscription', 'toggleIcon')
			),
			'show'   => array
			(
	...
	(
		...
		(
			...
			'inputType' => 'select',
			'options_callback' => array('tl_iso_subscription', 'getOrders'),
			'eval'    => array('tl_class' => 'w50', 'chosen' => true, 'includeBlankOption' => true),
			'wizard' => array
			(
				array('tl_iso_subscription', 'editOrder')
			),
			'sql'     => "int(10) unsigned NOT NULL default '0'"
		),
		'disable' => array
...
class tl_iso_subscription extends \Backend
{
	...
	public function setDateAdded(\DataContainer $objDc)
	{
		...
		\Database::getInstance()->prepare("UPDATE tl_iso_subscription SET dateAdded=? WHERE id=?")
	...
	public function checkPermission()
	{
		...
		switch (Input::get('act'))
		{
			...
			case 'toggle':
			case 'feature':
				$objArchive = $objDatabase->prepare("SELECT pid FROM tl_iso_subscription WHERE id=?")
					->limit(1)
					->execute($id);
			...
			case 'copyAll':
				...
				$objArchive = $objDatabase->prepare("SELECT id FROM tl_iso_subscription WHERE pid=?")
					->execute($id);
	...
	public function toggleVisibility($intId, $blnVisible)
	{
		...
		// Update the database
		$objDatabase->prepare("UPDATE tl_iso_subscription SET tstamp=". time() .", disable='" . ($blnVisible ? '' : 1) . "' WHERE id=?")
			->execute($intId);
				...
				while ($objOrders->next())
				{
					foreach ($objOrders->current()->getItems() as $objItem)
					{
						...
						if (($objProduct = $objItem->getProduct()) !== null &&
							$objProduct->type == $objSubscription->getRelated('pid')->productType)
						{
	...
	{
		return ($objDc->value < 1) ? '' : ' <a href="contao/main.php?do=iso_orders&act=edit&id=' . $objDc->value . '&rt=' . \RequestToken::get() . '" title="'.sprintf(specialchars($GLOBALS['TL_LANG']['tl_content']['editalias'][1]), $objDc->value).'" style="padding-left:3px">' . \Image::getHtml('alias.gif', $GLOBALS['TL_LANG']['tl_content']['editalias'][0], 'style="vertical-align:top"') . '</a>';
	}
